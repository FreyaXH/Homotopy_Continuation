...
def Genetic_Algorithm_Uncoupled(num_of_parents, num_iterations = 5, num_of_mutations = 5, tolerance_avrg = 0.1, survival_prob = 0.9,file_name = 'Genetic_Roots'):
    ...
    for i in range(num_of_parents):
        parents.append([np.random.uniform(1e4,2e5), np.random.uniform(1e4,2e5), np.random.uniform(1e4,2e5), np.random.uniform(0,7), np.random.uniform(0,7), \
                        np.random.uniform(0,7), 0,0,0,0,0,0,0,0,0])        
